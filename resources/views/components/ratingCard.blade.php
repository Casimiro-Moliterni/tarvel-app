    ...
    @if ($firstRating)
        <section id="star-values">
            <span class="star__container">
                <div class="rating_star">
                    @for ($i = 1; $i <= 5; $i++)
                        <label class="star__item {{ $i <= $firstRating->rating ? 'gold' : '' }}" for="star-{{ $event->id }}-{{ $i }}">
                            <span class="visuhide"><i class="fa-solid fa-star"></i></span>
                        </label>
                    @endfor
                </div>
            </span>
            <p>{{ $firstRating->review }}</p>
        </section>
    @endif
</div>
        ...
        <div class="center">
            <form data-url-rating="{{ route('admin.ratings.store') }}" class="form-rating d-flex flex-column">
                ...
                <fieldset class="rating ms-auto me-auto">
                    <input type="radio" id="star5" name="rating" value="5" class="star-input" /><label
                        for="star5" class="full" title="Awesome"></label>
                    <input type="radio" id="star4" name="rating" value="4" class="star-input" /><label
                    for="star4" class="full"></label>
                    <input type="radio" id="star3" name="rating" value="3" class="star-input" /><label
                        for="star3" class="full"></label>
                    <input type="radio" id="star2" name="rating" value="2" class="star-input" /><label
                        for="star2" class="full"></label>
                    <input type="radio" id="star1" name="rating" value="1" class="star-input" /><label
